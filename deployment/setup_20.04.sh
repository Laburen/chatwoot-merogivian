...
# Description: Install and manage a Merogivian installation.
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for Merogivian (e.g., chatwoot.domain.com): ' domain_name
...
serve Merogivian at https://$domain_name.
...
##############################################################################
# Setup Merogivian systemd services and cwctl CLI
# Globals:
#   None
# Arguments:
#   None
...
***************************************************************************
Woot! Woot!! Merogivian server installation is complete.
...
function cwctl_message() {
  echo $'\U0001F680 Try out the all new Merogivian CLI tool to manage your installation.'
...
***************************************************************************
              Merogivian Installation (v$CW_VERSION)
***************************************************************************
...
EOF
  ...
  sleep 3
  read -rp 'Would you like to configure a domain and SSL for Merogivian?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing Merogivian. This takes a long while."
...
function help() {
  ...
  cat <<EOF
Usage: cwctl [OPTION]...
Install and manage your Merogivian installation.
...
Installation/Upgrade:
  -i, --install             Install the latest stable version of Merogivian
  -I                        Install Merogivian from a git branch
  -u, --upgrade             Upgrade Merogivian to the latest stable version
...
Management:
  ...
  -l, --logs                View logs from Merogivian. Supported values include web/worker.
  -r, --restart             Restart Merogivian server
...
##############################################################################
# Get Merogivian web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function upgrade_redis() {
  ...
  echo "Upgrading Redis to v7+ for Rails 7 support(Merogivian v2.17+)"
...
##############################################################################
# Install pnpm - this replaces yarn starting from Merogivian 4.0
# Globals:
#   None
# Arguments:
#   None
...
function upgrade() {
  cwctl_upgrade_check
  get_cw_version
  echo "Upgrading Merogivian to v$CW_VERSION"
  sleep 3

   # Check if CW_VERSION is 4.0 or above
  if [[ "$(printf '%s\n' "$CW_VERSION" "4.0" | sort -V | head -n 1)" == "4.0" ]]; then
    echo "Merogivian v4.0 and above requires pgvector support in PostgreSQL."
  ...
  # Navigate to the Merogivian directory
  cd chatwoot
...
##############################################################################
# Restart Merogivian server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
