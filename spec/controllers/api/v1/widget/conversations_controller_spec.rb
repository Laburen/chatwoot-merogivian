...
RSpec.describe '/api/v1/widget/conversations/toggle_typing', type: :request do
  ...
  describe 'POST /api/v1/widget/conversations/set_custom_attributes' do
    let(:params) { { website_token: web_widget.website_token, custom_attributes: { 'product_name': 'Merogivian' } } }
    ...
    context 'with correct website token' do
      it 'sets the values when provided' do
        ...
        expect(response).to have_http_status(:success)
        conversation.reload
        # conversation custom attributes should have "product_name" key with value "Merogivian"
        expect(conversation.custom_attributes).to include('product_name' => 'Merogivian')
      end
    end
  end
  ...
  describe 'POST /api/v1/widget/conversations/destroy_custom_attributes' do
    ...
    context 'with correct website token' do
      it 'sets the values when provided' do
        ...
        conversation.custom_attributes = { 'product_name': 'Merogivian' }
        conversation.save!
        expect(conversation.custom_attributes).to include('product_name' => 'Merogivian')
        ...
        expect(response).to have_http_status(:success)
        conversation.reload
        # conversation custom attributes should not have "product_name" key with value "Merogivian"
        expect(conversation.custom_attributes).not_to include('product_name' => 'Merogivian')
      end
    end
  end
end
